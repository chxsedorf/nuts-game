...
function getStraightValues(card: Card): number[] {
  return card.rank === "A" ? [1, 14] : [card.value];
}

function canStep(prev: Card, next: Card, direction: 1 | -1): boolean {
  for (const prevValue of getStraightValues(prev)) {
    for (const nextValue of getStraightValues(next)) {
      if (nextValue - prevValue === direction) return true;
    }
  }
...
function isStraightCells(cells: LineCell[]): boolean {
  ...
  const asc = cards.every((card, index) =>
    index === 0 ? true : canStep(cards[index - 1], card, 1)
  );
  const desc = cards.every((card, index) =>
    index === 0 ? true : canStep(cards[index - 1], card, -1)
  );
...
function containsPlacedCell(cells: LineCell[], placedRow: number, placedCol: number) {
  return cells.some((cell) => cell.row === placedRow && cell.col === placedCol);
}
...
function detectFullHouse(cells: LineCell[], placedRow: number, placedCol: number): HandResult[] {
  if (cells.length !== BOARD_SIZE) return [];
  if (cells.some((cell) => !cell.card)) return [];
  if (!containsPlacedCell(cells, placedRow, placedCol)) return [];
  ...
  const countPattern = [...counts.values()].sort((a, b) => b - a);
...
function detectThrees(segments: LineCell[][], placedRow: number, placedCol: number): HandResult[] {
...
function detectStraights(segments: LineCell[][], placedRow: number, placedCol: number): HandResult[] {
  ...
  return candidates.filter((candidate) => {
    const candidateSet = new Set(candidate.cells);
...
function detectPairs(segments: LineCell[][], placedRow: number, placedCol: number): HandResult[] {
  const results: HandResult[] = [];
...
function evaluateLine(cells: LineCell[], placedRow: number, placedCol: number): HandResult[] {
  // Priority is strict per line: Full House -> Three -> Straight -> Pair.
  const fullHouse = detectFullHouse(cells, placedRow, placedCol);
  if (fullHouse.length > 0) return fullHouse;

  const segments = getContiguousCardSegments(cells);

  const threes = detectThrees(segments, placedRow, placedCol);
  if (threes.length > 0) return dedupeHandResults(threes);

  const straights = detectStraights(segments, placedRow, placedCol);
  if (straights.length > 0) return dedupeHandResults(straights);

  return dedupeHandResults(detectPairs(segments, placedRow, placedCol));
}
...
function evaluateBoard(board: Board, placedRow: number, placedCol: number): HandResult[] {
  return dedupeHandResults([
    ...evaluateLine(getRowCells(board, placedRow), placedRow, placedCol),
    ...evaluateLine(getColCells(board, placedCol), placedRow, placedCol),
  ]);
}
